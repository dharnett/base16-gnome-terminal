...
[[ -z "$PROFILE_NAME" ]] && PROFILE_NAME="Base 16 Seti UI 256"
[[ -z "$PROFILE_SLUG" ]] && PROFILE_SLUG="base-16-seti-256"
[[ -z "$DCONF" ]] && DCONF=dconf
...
if which "$DCONF" > /dev/null 2>&1; then
    ...
    if [[ -n "`$DCONF list $BASE_KEY_NEW/`" ]]; then
        ...
        dset cursor-background-color "'#d6d6d6'"
        dset cursor-foreground-color "'#151718'"
...
gset string cursor-background-color "'#d6d6d6'"
gset string cursor-foreground-color "'#151718'"
